...
`curl`.  However, in a normal derivation, you can't access the internet.[^2]  One
way to work around this is to create a separate fixed-output derivation, and
run the `curl` tests in that.  If `curl` hasn't been built correctly, and it can't
actually download files from the internet correctly, you can fail the Nix build.
