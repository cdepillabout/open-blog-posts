...
We're stuck on the `FFree` case again.  What is the type `_` this time?
...
```haskell
theFunctionWeNeedForBind :: (a -> FFree f b) -> f (FFree f) a -> f (FFree f) b
```

We might even be able to generalize this a to:

```haskell
theFunctionWeNeed :: Monad h => (a -> h b) -> f h a -> f h b
```

In our case, `f` will be `ExpF`, and `h` will be `FFree ExpF`.  Fully specialized, this would look like:

```haskell
theFunctionWeNeedSpecial :: (a -> FFree ExpF b) -> ExpF (FFree ExpF) a -> ExpF (FFree ExpF) b
```
...
instance Bound ExpF where
  ...
  ex >>>= f = case ex of
    ...
    ExpFLam mayb -> ExpFLam $ mayb >>= lift . f
```
...
3.  All the end-user would have to provide is the datatype `ExpF` and the
...
4.  `FFree` and `Bound` are _sort of_ like `Free` and `Functor` (except that
    GHC can automatically derive `Functor` in most cases, but the end-user would
...
and `IdentityT` used in `ExpF`, and define `Bound` for any `MonadTrans`.
...
The `Bound` instance then becomes [^7]:
...
instance
    ...
    , MonadTrans m
    ) => Bound (ExpF i m) where
  (>>>=) :: Monad h => ExpF i m h a -> (a -> h b) -> ExpF i m h b
  ex >>>= f = case ex of
    ExpFApp ida idb -> ExpFApp (ida >>= lift . f) (idb >>= lift . f)
    ExpFLam l -> ExpFLam $ l >>= lift . f
```
...
Honestly, using `FFree`, `Bound`, and `ExpF` doesn't save you _that_ much work
over just writing a `Monad` instance for `Exp`.  And `show`ing a value of
`FFree ExpF`
...
[^7]: The quantified constraints make this look a little complicated, but I expect
    ...
    available with GHC-9.6, since it has gained these exact quantified
    constraints on `MonadTrans`.
