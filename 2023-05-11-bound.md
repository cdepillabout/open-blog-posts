...
Edward Kmett show a bunch of different ways to express this in Haskell.
...
This should be straight-forward.  There are no `Maybe`s involved here.

How about the two cases of a single lambda expression, one with a free
...
`Exp` is nice because it has a straight-forward `Monad`, `Foldable`, and
...
modify free variables.  Here's an example of replacing free variables with
their ASCII codes:
