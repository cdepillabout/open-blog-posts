...
4.  `FFree` and `Bound` are _sort of_ like `Free` and `Functor`, except that
    GHC can automatically derive `Functor` in most cases, and the end-user would
...
- `FreeF (ExpTransF IdentityT MaybeT)` is isomorphic to `FreeF ExpF` and to `Exp`.
...
If there was an `FFree` library, it might contain a few nice helper functions,
